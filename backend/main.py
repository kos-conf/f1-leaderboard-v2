...
def notify_commentary_connections(commentary):
    """Notify all connected clients about commentary updates"""
    try:
        ...
        # Send to all connected clients
        disconnected = []
        for connection in list(commentary_connections):
            try:
                connection.put_nowait(message)
            except:
                disconnected.append(connection)
...
async def stream_commentary():
    """SSE endpoint for real-time commentary updates"""
    async def event_generator():
        # Create a queue for this connection
        queue = asyncio.Queue()
        ...
        try:
            # Send initial commentary if available
