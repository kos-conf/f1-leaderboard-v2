...
class RealisticPositionGenerator:
    def __init__(self, race_duration: int = 60):
        self.race_duration = race_duration  # 60 seconds
    ...
    def generate_race_positions(self, selected_driver_name: str, race_progress: float) -> List[Dict]:
        ...
        # Debug logging
        if race_progress > 0.9:  # Only log in final 10% to avoid spam
            print(f"DEBUG: {selected_driver_name} - Progress: {race_progress:.2f}, Starting: {starting_pos}, Target: {target_position}")
        ...
        if race_progress > 0.8:  # In the final 20% of the race
            if selected_driver_pos > 3 and target_position <= 3:
                ...
                if available_podium:
                    old_pos = selected_driver_pos
                    selected_driver_pos = random.choice(available_podium)
                    if race_progress > 0.9:  # Debug logging
                        print(f"DEBUG: {selected_driver_name} - Forced podium: {old_pos} -> {selected_driver_pos}")
